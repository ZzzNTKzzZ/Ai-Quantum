...
class RegimeAwarePortfolioEnv(gym.Env):
    def __init__(self, returns_df, regime_probs, weights_dim, horizon = 30, cost_rate = 0.001, reset_interval = 30, shock_interval = 30, shock_magnitude = -0.05):
        super().__init__()
...
# Chuyển đổi dữ liệu dài (long format) thành ma trận lợi nhuận (time x ticker)
returns_df = raw_df.pivot(index='time', columns='ticker', values='log_return').fillna(0)

# Lấy xác suất trạng thái thị trường (Market Regime) cho mỗi ngày
regime_probs = raw_df.groupby('time')[['Market_Prob_0', 'Market_Prob_1']].first().fillna(0)

# Cân bằng dữ liệu (đảm bảo số ngày của returns và regime khớp nhau)
returns_df, regime_probs = returns_df.align(regime_probs, axis=0, join='inner')

weights_dim = returns_df.shape[1]

print("Returns shape:", returns_df.shape)
print("Regime probs shape:", regime_probs.shape)

# Uncomment phần dưới đây để bắt đầu huấn luyện AI
